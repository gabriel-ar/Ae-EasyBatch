...
class Settings {
   ...
   */
  FromJson(json) {
    // Get all own property names from the JSON
    ...
    keys.forEach(key => {
      if (key === 'tmpls') {
        ...
        // Direct assignment preserves $state reactivity
        this[key] = json[key];
      }
    });
  }
  ...
  active_tab = $state<Tabs>(Tabs.Data);

  /** Mode of the data view, table or detail */
  data_mode = $state<"table" | "detail">("table");

  log_level = $state(Logger.Levels.Warn);

  /**
   * When the user Batch Renders, the intermediate compositions used to render are stored here
   */
  render_comps_folder = $state("~Automator Comps");

  out_mode = $state<OutMode>(OutMode.Render);

  auto_preview = $state(true);

  /**
   * At render time, only update the columns that are visible in the table
   */
  update_visible_col_only = $state(true);
...
class Template {
  ...
  save_pattern = $state("{base_path}/{template_name}_{row_number}");

  render_setts_templ = $state("");
  render_out_module_templ = $state("");

  /** Pattern used to name generated compositions */
  generate_pattern = $state("Generated_{increment:0000}");

  /** Array of save paths calculated from the save pattern */
  save_paths = $state<string[]>([]);

  generate_names = $state([]);

  /**When importing footage to fulfill a replaceable, we place it in this folder inside the project */
  imported_footage_folder = $state("~Imported by EasyBatch");

  /** When generating comps, we place them in this folder */
  gen_comps_folder = $state("~Generated by EasyBatch");
  ...
  static FromJson(json) {
    ...
    keys.forEach(key => {
      if (key === 'columns') {
        ...
        // rows will be recalculated, skip direct assignment
      } else if (key in templ) {
        // Direct assignment preserves $state reactivity
        templ[key] = json[key];
      }
    });
...
class Column {
   ...
   * @type {PropTypeType}
   */
  type = $state<number>(6424); // TEXT_DOCUMENT
  ...
  /**
   * Creates a Column object from JSON
   * @param {Column} json
   */
  static FromJson(json) {
    let col = new Column();
